...
(function (J, D) {
    ...
    var defaultOpts = {
        url:'/',
        type:'json',//ajax is json or jsonp
        dataKey:'',
        filterHtml: true,
        autoSubmit: true,
        forceClear:false,
  ...
  //      allowEmpty:false,//是否允许空值触发ａｊａｘ
     ...
     * @constructor
     */
    function AutocompleteV2(el, options) {
         ...
         */
        function sendData(dataJson,handler){
            var len = AutocompleteCallback.length;
            var callbackHandler;
            len&&(AutocompleteCallback[len-1]= function(){});
            var ajaxSetting = {
                type:opts.type,
                data:dataJson,
                url:opts.url
            ...
            if(opts.cache){
                ...
                for(var i in dataJson){
                    cacheKey= cacheKey+ dataJson[i];
                }
                if(CACHED[cacheKey]){
                    handler(CACHED[cacheKey]);
                    return;
                }
            }

            if(opts.type==='json'){
                ajaxSetting.onSuccess = AutocompleteCallback[len] = (function(){
            ...
            if(opts.type==='jsonp'){
                AutocompleteCallback[len] = (function(){
                    return function(data){
                        CACHED[cacheKey]=data;
                        handler(data);
                    }
                })();
                ajaxSetting.callback = 'AutocompleteCallback['+len+']';
            }
            J.get(ajaxSetting);
        }
    ...
    J.dom.fn.autocompleteV2 = function(options){
        return new AutocompleteV2(this.get(), options)
    };
    J.ui.autocompleteV2 = AutocompleteV2;
})(J, document);